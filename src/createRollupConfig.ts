...
export async function createRollupConfig(
	...
	return {
		...
		plugins: [
			...
			// TODO: Compare perf when using @rollup/plugin-typescript
			// typescript({
			// 	typescript: ts,
			// 	tsconfig: opts.tsconfig,
			// 	sourceMap: true,
			// 	jsx: (tsCompilerOptions?.jsx as any) ?? "react",
			// 	declaration:
			// 		outputNum > 0 ? false : tsCompilerOptions?.declaration ?? true,
			// 	declarationDir:
			// 		outputNum > 0
			// 			? undefined
			// 			: tsCompilerOptions?.declarationDir ?? paths.appDist,
			// 	declarationMap:
			// 		outputNum > 0 ? false : tsCompilerOptions?.declaration ?? true,
			// 	target: "esnext",
			// 	exclude: [
			...
			// 		paths.appDist,
			// 	],
			// 	noEmitOnError: !opts.transpileOnly && outputNum === 0,
			// }),
			babelPluginTsdx({
